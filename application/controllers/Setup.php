...
class Setup extends CI_Controller
{
    ...
    public function edit_customer($customer_id)
    {
        ...
        $data['contact_data'] = $this->Setup_model->get_customer_contacts($customer_id);
    ...
    {
        $result = $this->Setup_model->update_customer($customer_id);
    ...
    {
        $result = $this->Setup_model->delete_customer($customer_id);
    ...
    public function add_item()
    {
        ...
        $data['product_types'] = $this->Setup_model->get_all_product_types();
...
>>>>>>> 6c468d3 (sales module changes)
            ...
            'retail_price'       => $this->input->post('retail_price', true) ?: 0,
            'cost_price'         => $this->input->post('cost_price', true) ?: 0,

            'min_level'          => $this->input->post('min_level', true) ?: 0,
            'max_level'          => $this->input->post('max_level', true) ?: 0,
            'reorder_level'      => $this->input->post('reorder_level', true) ?: 0,
        ...
        if ($insert_id) {
            $this->Setup_model->save_item_raw_materials($insert_id, $product_type_id);
    ...
    public function update_item()
    {
        ...
        if (!empty($_FILES['product_image']['name'])) {
            ...
            if ($this->upload->do_upload('product_image')) {
                ...
                if (
                    !empty($existing_item['product_image']) &&
        ...
        $this->Setup_model->update_item_raw_materials($item_id, $product_type_id);
...
>>>>>>> 6c468d3 (sales module changes)
    ...
    public function list_warehouse()
    {
        ...
        if (!has_view_access($user, 'Setup/list_warehouse')) {
            $data['title'] = 'Access Denied';
            $data['main_content'] = 'errors/access_control.php';
        } else {
            $data['title'] = 'Warehouse Master';
            $data['warehouses'] = $this->Setup_model->get_all_warehouses();
            $data['main_content'] = 'setup/list_warehouse.php';
        }
    ...
    public function add_warehouse()
    {
        ...
        if (!has_access($user, 'Setup/list_warehouse', 'A')) {
            $data['title'] = 'Access Denied';
            $data['main_content'] = 'errors/access_control.php';
        } else {
            $data['title'] = 'Add Warehouse';
            $data['branches'] = $this->Setup_model->get_all_branches();
            $data['main_content'] = 'setup/warehouse_form.php';
        }

        $this->load->view('includes/template', $data);
    }

    public function add_warehouse_data()
    {
        $result = $this->Setup_model->insert_warehouse();

        if ($result) {
            $this->session->set_flashdata('success', 'Warehouse Added Successfully');
        } else {
            $this->session->set_flashdata('error', 'Warehouse Name Already Exists');
        }

        redirect('Setup/list_warehouse');
    }
    ...
    public function edit_warehouse($id)
    {
        ...
        if (!has_access($user, 'Setup/list_warehouse', 'E')) {
            $data['title'] = 'Access Denied';
            $data['main_content'] = 'errors/access_control.php';
        } else {

            $data['title'] = 'Edit Warehouse';
            $data['warehouse'] = $this->Setup_model->get_warehouse_by_id($id);
            $data['branches'] = $this->Setup_model->get_all_branches();
            $data['main_content'] = 'setup/warehouse_form.php';
        }
    ...
    public function update_warehouse_data()
    {
        $result = $this->Setup_model->update_warehouse();
    ...
    {
        $id = $this->input->post('id');
        $result = $this->Setup_model->delete_warehouse($id);
    ...
    public function list_store()
    {
        ...
        if (!has_view_access($user, 'Setup/list_store')) {
            $data['title'] = 'Access Denied';
            $data['main_content'] = 'errors/access_control.php';
        } else {
            $data['title'] = 'Store Master';
            $data['stores'] = $this->Setup_model->get_all_stores();
            $data['main_content'] = 'setup/list_store.php';
        }
    ...
    public function add_store()
    {
        ...
        if (!has_access($user, 'Setup/list_store', 'A')) {
            $data['title'] = 'Access Denied';
            $data['main_content'] = 'errors/access_control.php';
        } else {
            $data['title'] = 'Add Store';
            $data['warehouses'] = $this->Setup_model->get_all_warehouses();
            $data['main_content'] = 'setup/store_form.php';
        }
    ...
    {
        $result = $this->Setup_model->insert_store();

        if ($result) {
            $this->session->set_flashdata('success', 'Store Added Successfully');
        } else {
            $this->session->set_flashdata('error', 'Store Already Exists');
        }

        redirect('Setup/list_store');
    }
    ...
    public function edit_store($id)
    {
        ...
        if (!has_access($user, 'Setup/list_store', 'E')) {
            $data['title'] = 'Access Denied';
            $data['main_content'] = 'errors/access_control.php';
        } else {
            $data['title'] = 'Edit Store';
            $data['store'] = $this->Setup_model->get_store_by_id($id);
            $data['warehouses'] = $this->Setup_model->get_all_warehouses();
            $data['main_content'] = 'setup/store_form.php';
        }
    ...
    {
        $result = $this->Setup_model->update_store();
    ...
    {
        $id = $this->input->post('id');
        $result = $this->Setup_model->delete_store($id);
    ...
    public function update_terms_conditions_data()
    {
        ...
        // DUPLICATE CHECK
        $exists = $this->Setup_model ->check_terms_conditions_duplicate($term_type, $terms_name, $terms_id);
        if ($exists > 0) {
            $this->session->set_flashdata('warning','This Terms & Conditions already exists for the selected Term Type.');
            redirect('Setup/edit_terms_conditions/' . $terms_id);
            return;
    ...
    public function delete_terms_conditions($id)
    {
        ...
        if ($result) {
            ...
            $this->session->set_flashdata(
                'error',
                'Unable To Deactivate Terms & Conditions'
            );
        }
